...
class SaleWizard(models.TransientModel):
    ...
    student_ids = fields(comodel_name='res.partner', string='Students for sales order')
        ...
        if(session_product_id):
            for(student in self.student_ids):
                order_id = self.env['sale.order'].create({
                    ...
                    'order_line': [(0,0,{'product_od': session_product_id.id, 'price_unit': self.session_id.total_price})]
                })
